...
Pipeline::Pipeline(Device& device, const gfx::PipelineInfo& info) {
  this->m_device = &device;

  LunaAssert(!info.spirv.empty());
  this->m_shader = std::make_unique<Shader>(device, info.spirv);
...
Pipeline::Pipeline(RenderPass& pass, const gfx::PipelineInfo& info)
{
  this->init_params();
  ...
  this->m_shader = std::make_unique<Shader>(*this->m_device, info.spirv);
...
auto Pipeline::parse(const gfx::PipelineInfo& info) -> void {
  if (info.depth_test) {
