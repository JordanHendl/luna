...
class Shader {
 public:
  ...
  explicit Shader();
  explicit Shader(const std::vector<std::pair<Type, std::vector<uint32_t>>>& spv_to_load);
  explicit Shader(const std::vector<std::string>& glsl_to_load);
  explicit Shader(std::vector<std::pair<std::string, std::string>> inline_files, std::vector<std::string> include_dirs = {});
