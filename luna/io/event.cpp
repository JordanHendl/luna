...
auto convert(SDL_Event& event) -> luna::Event {
  auto lib_event = luna::Event();

  switch (event.type) {
    case SDL_KEYDOWN: {
      lib_event = luna::Event(luna::Event::Type::KeyDown,
                             keyFromSDL(event.key.keysym.sym));
      break;
    }
    case SDL_KEYUP: {
      lib_event =
          luna::Event(luna::Event::Type::KeyUp, keyFromSDL(event.key.keysym.sym));
      break;
    }
    case SDL_MOUSEBUTTONDOWN: {
      lib_event = luna::Event(luna::Event::Type::MouseButtonDown,
                             mouseButtonFromSDL(event.button.button));
      break;
    }
    case SDL_MOUSEBUTTONUP: {
      lib_event = luna::Event(luna::Event::Type::MouseButtonUp,
                             mouseButtonFromSDL(event.button.button));
      break;
    }
    case SDL_QUIT: {
      lib_event = luna::Event(luna::Event::Type::WindowExit, Key::ESC);
      break;
    }
    default:;
  }
...
auto EventRegister::add(std::function<void(const luna::Event&)> cb) -> void {
